...
def solution(inputFile):
                ...
                for j in range(len(scanners)):
                    ...
                    if scanner2.corrected == False and len(genDistanceIntersection(scanner1,scanner2))==66:
                        ...
                        for translationIndex in range(-1,23):
                            ...
                            if (maxInfo[0]>=12):
                                # log('Found rotation', translationIndex, maxInfo[1], maxInfo[2])
    ...
    differentBeacons = set()

    for scanner in scanners:
        for beacon in scanner.beacons:
            differentBeacons.add('%s%s%s' % (beacon.offsetX,beacon.offsetY,beacon.offsetZ))

    result = len(differentBeacons)
