...
def solution(inputFile):
    ...
    def stabilizeSFN(sFN):
        stable = False
        while not stable:
            stable = True

            explodeResult = resolveExplosion(sFN)
            if explodeResult[0]:
                sFN = explodeResult[1]
                stable = False
    ...
    for i in range(len(inputData)):
        for j in range(len(inputData)):
            if i!=j:
                sFNSum = stabilizeSFN(add(inputData[i], inputData[j]))
                maxMagnitude = max(maxMagnitude, calculateMagnitude(sFNSum))

    return (maxMagnitude,4626)
