...
def solution(inputFile):
    ...
    for galaxyStartIndex in range(len(galaxyLocations)-1):
        for galaxyStopIndex in range(galaxyStartIndex+1, len(galaxyLocations), 1):
            ...
            distance += sum([EXPANSION_RATE-1 if emptyLine>=minY and emptyLine<=maxY else 0 for emptyLine in emptyLines])
            distance += sum([EXPANSION_RATE-1 if emptyColumn>=minX and emptyColumn<=maxX else 0 for emptyColumn in emptyColumns])
